...
function BookingSchedule() {
  ...
  const renderDaySchedule = (title: string, dayBooks: MeetingBook[]) => (
    ...
    >
      {dayBooks.length === 0 ? (
        <Text type="secondary" style={{ fontSize: 12 }}>暂无会议预定</Text>
      ) : (
        dayBooks.map((book) => {
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <Tag color="blue" style={{ fontSize: 10, lineHeight: '16px', margin: 0 }}>{book.roomName}</Tag>
                <Text strong style={{ fontSize: 12 }}>{book.meetingName}</Text>
                <Text type="secondary" style={{ fontSize: 11 }}>
                  <UserOutlined style={{ marginRight: 2 }} />{book.bookerName}
                </Text>
                <Tag color={status.color} style={{ fontSize: 10, lineHeight: '14px', margin: 0, marginLeft: 'auto' }}>
                  {status.text}
                </Tag>
              </div>
            </div>
  ...
  const renderMyBooks = () => (
    <Card
      ...
      size="small"
      styles={{ body: { padding: '8px 12px', maxHeight: 320, overflowY: 'auto' } }}
    >
      {myBooks.length === 0 ? (
        <Text type="secondary" style={{ fontSize: 12 }}>暂无预约</Text>
      ) : (
        myBooks.map((book) => {
          const status = getStatusTag(book)
          ...
          return (
            <div
              ...
              style={{
                padding: '6px 8px',
                marginBottom: 4,
                borderLeft: `3px solid ${isCanceled ? '#d9d9d9' : status.color === 'blue' ? '#1677ff' : status.color === 'orange' ? '#fa8c16' : '#d9d9d9'}`,
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <Tag color="blue" style={{ fontSize: 10, lineHeight: '16px', margin: 0 }}>{book.roomName}</Tag>
                <Text strong style={{ fontSize: 12 }}>{book.meetingName}</Text>
                <Tag color={isCanceled ? 'default' : status.color} style={{ fontSize: 10, lineHeight: '14px', margin: 0 }}>
                  {isCanceled ? '已取消' : status.text}
                </Tag>
                <div style={{ marginLeft: 'auto' }}>
                  {book.isFinished !== '1' && new Date(book.meetingStartTime).getTime() > Date.now() && (
                    isCanceled ? (
                      <Button size="small" type="link" style={{ fontSize: 10, padding: 0 }} onClick={() => handleRestore(book.bookId, book.bookerId)}>
                        恢复
                      </Button>
                    ) : (
                      <Button size="small" type="link" danger style={{ fontSize: 10, padding: 0 }} onClick={() => handleCancel(book.bookId, book.bookerId)}>
                        取消
                      </Button>
                    )
                  )}
  ...
  return (
    <Card
      ...
      cover={
        imgSrc ? (
          ...
          >
            <EnvironmentOutlined style={{ fontSize: 22, color: '#d9d9d9' }} />
          </div>
        )
      }
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        <Text strong style={{ fontSize: 11 }}>{room.name}</Text>
...
function MeetingDesktop() {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', height: '100%', overflow: 'hidden' }}>
      {/* 左侧：会议预定日程 */}
      <div style={{ width: 380, flexShrink: 0, padding: '12px 8px 12px 12px', borderRight: '1px solid #f0f0f0' }}>
        <Title level={5} style={{ margin: '0 0 10px 0' }}>会议预定日程</Title>
        <BookingSchedule />
      </div>

      {/* 右侧：会议室卡片 */}
      <div style={{ flex: 1, padding: 14, overflowY: 'auto' }}>
        <Title level={5} style={{ margin: '0 0 10px 0' }}>会议室列表</Title>
        {loading ? (
          <Spin tip="加载中...">
