...
function BookingSchedule() {
  ...
  const renderDaySchedule = (title: string, dayBooks: MeetingBook[]) => (
    <Card
      ...
      styles={{ body: { padding: '6px 8px', maxHeight: 200, overflowY: 'auto' } }}
    >
      {dayBooks.length === 0 ? (
        <Text type="secondary" style={{ fontSize: 10 }}>暂无会议预定</Text>
      ) : (
        dayBooks.map((book) => {
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
                ...
                <Tag color="blue" style={{ fontSize: 9, lineHeight: '14px', margin: 0 }}>{book.roomName}</Tag>
  ...
  const renderMyBooks = () => (
    <Card
      ...
      size="small"
      styles={{ body: { padding: '6px 8px', maxHeight: 240, overflowY: 'auto' } }}
    >
      {myBooks.length === 0 ? (
        <Text type="secondary" style={{ fontSize: 10 }}>暂无预约</Text>
      ) : (
        myBooks.map((book) => {
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 2 }}>
                  {book.isFinished !== '1' && new Date(book.meetingStartTime).getTime() > Date.now() && (
                    isCanceled ? (
                      <Button size="small" type="link" style={{ fontSize: 9, padding: 0, height: 18 }} onClick={() => handleRestore(book.bookId, book.bookerId)}>
                        恢复
                      </Button>
                    ) : (
                      <Button size="small" type="link" danger style={{ fontSize: 9, padding: 0, height: 18 }} onClick={() => handleCancel(book.bookId, book.bookerId)}>
                        取消
                      </Button>
                    )
                  )}
...
function MeetingTablet() {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', height: '100%', overflow: 'hidden' }}>
      {/* 左侧：会议预定日程 */}
      <div style={{ width: 280, flexShrink: 0, padding: '10px 6px 10px 10px', borderRight: '1px solid #f0f0f0' }}>
        <BookingSchedule />
      </div>
